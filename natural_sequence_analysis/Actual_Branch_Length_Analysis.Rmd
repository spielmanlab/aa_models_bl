...
## The Data Pivoted wide to show all models and pivoted again to compare Poisson branchlengths with all others 
...
input_data%>%
pivot_longer( c(FLU_FALSE,FLU_TRUE,JTT_FALSE, JTT_TRUE, LG_FALSE, LG_TRUE, Poisson_TRUE, WAG_FALSE, WAG_TRUE), names_to = "model_type", values_to = "branchlength")->input_poisson_f
...
input_poisson_f%>%
  ...
  tidyr::nest()%>%
  mutate(lm_fit=map(data, ~lm(Poisson_FALSE~branchlength, data=.)))%>%
  select(-data)%>%
  unnest(lm_fit)->Testing123
