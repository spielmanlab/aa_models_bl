...
#linear model with three different types, each with different predictors. Once a lm is performed, do an aov and tukey test for significance and report the results as a table where the models will be compared. Significant p values shown in red. Also include a line_range plot to visualize the tukey comparisons.  The three arguments for this function are the branch length dataframe to be used, the type of model to run, and which summary statistic to run the model on. 
...
{
list("Chart"= Table_output, "Plot"=Plot_output)->plot_and_chart_output
  print(plot_and_chart_output)
}}
